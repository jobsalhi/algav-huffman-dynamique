...
Une modification (mise à jour après lecture d'un symbole, c'est-à-dire d'un
...
cas, $L=k+1$ et le nombre total de nœuds $n$ vérifie :
\[
    n = 2(k+1)-1 = 2k+1 \le 511.
\]
...
``feuille $\rightarrow$ racine'', de longueur $p$ (profondeur de la feuille
...
Ainsi, le \textbf{nombre maximal d'échanges par mise à jour} est en $O(h)$, où h est la hauteur de l'arbre. 
Dans le cas d'un arbre dégénéré (pire cas), on a la hauteur $h$ qui est égale au nombre de
symboles distincts déjà rencontrés, c'est-à-dire $k$, ce qui nous donne \textbf{$O(k)$}.
Cette borne est volontairement pessimiste, l'arbre de Huffman dynamique n'étant pas garanti équilibré.
...
Nous avons implémenté \texttt{utils/bitreader.py} de la façon suivante : 
\begin{enumerate}
    \item lecture du fichier en mode \texttt{rb}
    \item conversion de chaque octet en représentation binaire sur 8 bits, concaténée en une chaîne de \texttt{'0'} et \texttt{'1'}.
\end{enumerate}

\subsection*{Question 4 — Fonction \texttt{ecriture(fichier\_chaine.txt, fichier.bin)} : bits $\rightarrow$ binaire}
Nous avons implémenté \texttt{utils/bitwriter.py} comme ceci : 
\begin{enumerate}
    \item lecture de la chaîne de bits
    \item padding à droite pour obtenir un multiple de 8
    \item conversion par blocs de 8 bits en octets
    \item écriture en mode \texttt{wb}
\end{enumerate}
Le résultat est lisible par \texttt{lecture}.
...
\begin{itemize}
  ...
  \item mise à jour (remontée + échanges) : $O(hn)$ (explications dans la \hyperref[comp_echange]{question 2}) ;
...
Pour la compression, nous avons suivi le processus suivant:
\begin{enumerate}
    \item création d'un arbre vide 
    \item lecture du fichier UTF-8 en format binaire 
    \item écriture de la taille du fichier en binaire sur 64 bits dans un fichier temporaire
    \item boucle sur chaque octet lu du fichier :
    \begin{itemize}
        \item 
        \begin{itemize}
            \item[$\checkmark$] si le symbole lu existe déjà, on récupère son chemin
            \item[$\checkmark$] sinon on récupère le chemin menant du noeud NYT, puis on le concatène au code binaire du symbole lu
        \end{itemize}
    \item écriture du chemin du symbole dans le fichier temporaire
    \item mise à jour de l'arbre
    \end{itemize}
    \item appel à la fonction écriture() pour recopier le contenu du fichier temporaire dans le fichier de sortie .huff
\end{enumerate}

\paragraph{Décompression}
La décompression suit la même logique :
\begin{enumerate}
    \item création d'un arbre vide 
    \item lecture de la taille du fichier dans les 64 bits d'en-tête
    \item lecture du fichier .huff avec la fonction lecture() 
    \item boucle sur les bits de la séquence récupérée :
    \begin{itemize}
        \item 
        \begin{itemize}
            \item[$\checkmark$] si on a le chemin menant du nœud NYT, on récupère les 8 bits suivants
            \item[$\checkmark$] sinon, on a le chemin menant à une feuille, et on récupère son symbole
        \end{itemize}
    \item écriture en binaire du symbole dans le fichier de sortie
    \item mise à jour de l'arbre
    \end{itemize}
\end{enumerate}
\paragraph{}
...
Premièrement nous avons testé \texttt{io/input/emojis.txt} qui nous a permet de valider la compression de symboles de plusieurs octets (1 emoji = 4 octets)
...
Le tableau~\ref{tab:results} synthétise les résultats observés
(issus de \texttt{tests/compression\_report.csv}).
...
\csvreader[
  ...
  column names={
    ...
    decompression_ms=\decompressionms
  },
  late after line=\\
]{tests/compression_report.csv}{}%
{%
\file &
\originalsize &
\compressedsize &
...
La proportion de code généré ou modifié avec l'aide de l'IA est estimée à environ \textbf{30\,\%}, et la proportion du texte du rapport influencée par l'IA à environ \textbf{40\,\%}.
